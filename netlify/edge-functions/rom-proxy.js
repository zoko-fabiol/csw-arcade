export default async function handler(request, context) {
  ...
  // Liste de sources Archive.org pour maximiser la disponibilité
  let candidateUrls = [];
  if (customUrl) {
  ...
  return new Response(JSON.stringify({ error: 'ROM introuvable sur les serveurs Archive.org', details: lastError?.message }), {
    status: 404,
    headers: {
      'Content-Type': 'application/json',
      'Access-Control-Allow-Origin': '*'
